...
pio run -t upload           # flash the firmware — the web UI is compiled in
```

The web interface is **embedded in the firmware image**, so there is no separate
filesystem upload to forget. Uploading LittleFS (`./sync_web_data.sh` then
`pio run -t uploadfs`) is an optional per-file override, for changing the
interface on a device without recompiling.
